...
export class CasaOSClient {
  private config: CasaOSConfig;

  constructor(config: CasaOSConfig) {
    this.config = {
      port: 22,
      ...config,
    };
  ...
  public async sshExec(command: string): Promise<{ stdout: string; stderr: string; code: number }> {
  ...
  public async deployCasaOSApp(params: {
    appId: string;
    title: string;
    icon: string;
    webPort?: number;
...
`;
  ...
  public async convertLegacyApp(containerName: string, iconUrl?: string, titleName?: string): Promise<string> {
...
`;
  ...
  public async setAppIcon(appId: string, iconUrl: string, title?: string): Promise<string> {
    const script = `python3 -c "
import yaml, os
...
`;
  ...
  public async manageApp(appId: string, action: 'start' | 'stop' | 'restart'): Promise<string> {
        ...
        res.on('end', () => {
          try {
            ...
            const icons = (data.icons || []).map(
              (name: string) => `https://api.iconify.design/${name.replace(':', '/')}.svg`
            );
            resolve(icons);
          } catch {
            resolve([]);
  ...
  public async getAppLogs(appId: string, lines: number = 50): Promise<string> {
...
"`;
  ...
  public async readAppConfig(appId: string): Promise<any> {
    const script = `python3 -c "
import yaml, json, os
...
"`;
  ...
  public async updateAppEnv(appId: string, envUpdates: Record<string, string>): Promise<string> {
    const script = `python3 -c "
import yaml, os, json
...
`;
  ...
  public async backupAppData(appId: string): Promise<string> {
    const script = `python3 -c "
...
"`;
  ...
  public async manageAppStore(action: 'add' | 'remove' | 'list', url?: string): Promise<string> {
    if (action === 'add' && url) {
  ...
  public async listStorage(): Promise<string> {
